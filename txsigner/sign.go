...
//export SignWithdraw
func SignWithdraw(cUSDCAmount int64, cNonce int64, cApiKeyIndex int32, cAccountIndex int64) (ret SignedTxResponse) {
	defer func() {
		if r := recover(); r != nil {
			ret = signedTxResponsePanic(fmt.Errorf("%v", r))
		}
...
//export SignTransfer
func SignTransfer(cToAccountIndex int64, cUSDCAmount int64, cFee int64, cMemo string, cNonce int64, cApiKeyIndex int32, cAccountIndex int64) (ret SignedTxResponse) {
	defer func() {
		if r := recover(); r != nil {
			ret = signedTxResponsePanic(fmt.Errorf("%v", r))
		}
	...
	memoStr := cMemo
	if len(memoStr) != 32 {
		return signedTxResponseErr(errors.New("memo expected to be 32 bytes long"))
	}
	for i := 0; i < 32; i++ {
...
func SignCreatePublicPool(cOperatorFee int64, cInitialTotalShares int64, cMinOperatorShareRate int64, cNonce int64, cApiKeyIndex int32, cAccountIndex int64) (ret SignedTxResponse) {
	defer func() {
		if r := recover(); r != nil {
			ret = signedTxResponsePanic(fmt.Errorf("%v", r))
		}
...
func SignUpdatePublicPool(cPublicPoolIndex int64, cStatus int32, cOperatorFee int64, cMinOperatorShareRate int64, cNonce int64, cApiKeyIndex int32, cAccountIndex int64) (ret SignedTxResponse) {
	defer func() {
		if r := recover(); r != nil {
			ret = signedTxResponsePanic(fmt.Errorf("%v", r))
		}
